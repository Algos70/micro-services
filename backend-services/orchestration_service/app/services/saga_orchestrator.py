...
class SagaOrchestrator:
    ...
    def start_order_saga(self, order_id: str, order_data: dict):
        ...
        self.saga_store[order_id] = saga_state
    ...
    def handle_stock_reduced_event(self, order_id: str):
        # Update saga state
        saga_state = self.saga_store.get(order_id)
        if not saga_state:
    ...
    def handle_insufficient_stock_event(self, order_id: str):
    ...
    def handle_payment_verified_event(self, order_id: str):
        # Next step: publish "CreateOrderCommand"
        saga_state = self.saga_store.get(order_id)
        saga_state.current_step = "CREATE_ORDER"
        self.publisher.publish_create_order_command(order_id, saga_state.order_data)

    def handle_payment_unverified_event(self, order_id: str):
        # Trigger rollback stock if needed
        saga_state = self.saga_store.get(order_id)
        saga_state.status = "FAILED"
        self.publisher.publish_rollback_stock_command(order_id, ...)

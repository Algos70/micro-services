...
export function SignInForm({ type }: SignInFormProps) {
  ...
  return (
    <div /* Container */ className="min-h-screen flex">
      ...
      <Alert
        variant="default"
        className={`fixed top-2 left-1/3 max-w-1/3 flex flex-col items-center transition-opacity duration-500 ${
          signInSuccess ? 'opacity-100 visible' : 'opacity-0 invisible'
      ...
      >
        ...
        <AlertDescription className="text-black">Signed In successfully</AlertDescription>
      </Alert>
      ...
      <div /* Content */ className="w-1/2 flex justify-center items-center bg-primary-900 text-white">
        <div className="relative max-w-md w-full bg-white text-gray-900 shadow-lg rounded-lg p-8">
          <div
            className={`absolute inset-0 text-center flex flex-col items-center gap-2 bg-violet-900 text-gray-50 dark:bg-violet-900 dark:text-gray-50 duration-500 transition-opacity  ${
              signUpSuccess ? 'visible opacity-100' : 'invisible opacity-0'
            }`}
          >
            ...
            <Button onClick={handleConfirmClick} className="mt-2 shadow-sm active:shadow-lg transition duration-100">
              Confirm
            </Button>
            <p className="mt-2">
              Back to{' '}
              <a href={'/'} className="underline underline-offset-2 cursor-pointer text-violet-50">
                Homepage
              </a>
            </p>
          </div>
          <h1 className="text-2xl font-extrabold text-center">{type === 'sign-in' ? 'Sign In' : 'Sign Up'}</h1>
          <div className="mt-6">
            <Form {...form}>
              <form className="space-y-6" onSubmit={type === 'sign-up' ? form.handleSubmit(onSubmitSignUp) : form.handleSubmit(onSubmitSignIn)}>
                <FormField
